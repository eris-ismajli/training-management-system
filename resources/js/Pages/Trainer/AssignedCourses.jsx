...
export default function AssignedCourses({ courses: initialCourses, auth }) {
    ...
    const enrollStudent = async (courseId, email) => {
        ...
        setCourses(prev =>
            prev.map(course =>
                course.id === courseId
                    ? { ...course, students: [...(course.students || []), data.student] }
                    : course
            )
        );
        ...
        <AuthenticatedLayout>
            ...
            <div className="flex min-h-screen">
                ...
                <main className="flex-1 py-12 px-8 max-w-6xl mx-auto space-y-6">
                    <h2 className="text-2xl font-bold text-gray-800">Your Assigned Courses</h2>

                    {courses.length === 0 ? (
                        <p className="text-gray-600">You have no assigned courses.</p>
                    ) : (
                        <div className="space-y-10">
                            {courses.map(course => (
                                <div key={course.id} className="bg-white rounded-xl shadow p-6 space-y-6 border border-gray-200">
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-sm text-gray-600">Category: <span className="font-medium">{course.category}</span></p>
                                        <p className="text-sm text-gray-600">Description: {course.description || "N/A"}</p>
                                    </div>

                                    <form
                                        onSubmit={(e) => {
                                            e.preventDefault();
                                            const email = searchInputs[course.id] || "";
                                            enrollStudent(course.id, email);
                                        }}
                                        className="space-y-2"
                                    >
                                        <label className="text-sm font-medium text-gray-700">Enroll Student by Email:</label>
                                    ...
                                    {course.students?.length > 0 && (
                                        <div className="space-y-3">
                                            <p className="text-sm font-semibold text-gray-700">Enrolled Students:</p>
